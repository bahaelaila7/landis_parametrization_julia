...
const OUT = "runs/fig_fl5_species_panels_FL5EXTENT.png"
# FL5 extent = geographic bbox of the FL5 TreeMap raster (FL5_22.tif) in EPSG:4326.
const FL5_BOX = (lon0 = -83.42, lon1 = -81.982, lat0 = 29.758, lat1 = 30.699)

# restrict to plots physically inside the FL5 raster extent (data_plot_eco lon/lat)
excon = DBInterface.connect(DuckDB.DB(cfg["cohorts_db_path"]))
inbox = DataFrame(DBInterface.execute(excon, """
...
println("FL5-extent plots (lon/lat in box): ", length(fplots))
...
splots, eco_list, species_list, eco_species_ids, _ = D.prepare_parametrization_data(;
  ...
  filter_ecos=String.(get(cfg, "filter_ecos", String[])), filter_plots=fplots, RNG=rng)
