...
const OUT = "runs/fig_fl5_map_CORRECTED.png"
...
# CORRECTED: land_use = FIA STDORGCD ground truth, per plot. natural iff EVERY forested condition has
# STDORGCD=0; any STDORGCD=1 (ever planted) → artificial  → MAX(STDORGCD)=1.
plots = DataFrame(DBInterface.execute(con, """
  ...
  SELECT CAST(e.lon AS DOUBLE) lon, CAST(e.lat AS DOUBLE) lat, MAX(s.lu) land_use
...
ax = MK.Axis(fig[1, 1]; xlabel = "lon", ylabel = "lat", title = "FL5 area — AGB (Live), EPA L3/L4, counties, FIA plots by STDORGCD origin (planted→artificial)",
