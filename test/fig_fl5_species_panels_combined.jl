...
const OUT = "runs/fig_fl5_species_panels_combined.png"
...
println("COL1 EPA FIA plots:", length(unique(splots1.plot_id)))
...
println("COL2 FL5 FIA plots:", length(unique(splots2.plot_id)))
...
fl5 = DataFrame(DBInterface.execute(con, """
  ...
  SELECT c.epa_l3 l3, c.land_use lu, UPPER(TRIM(c.species_symbol)) sym, c.spgrpcd grp,
...
println("COL3 FL5 TreeMap cohort rows:", nrow(fl5))

# --- render 4×3 grid ---
STRATA = [("8.3.5", "artificial"), ("8.3.5", "natural"), ("8.5.3", "artificial"), ("8.5.3", "natural")]
COLS = [("EPA FIA  (Σ AGB)", comp1), ("FL5 FIA  (Σ AGB)", comp2), ("FL5 TreeMap  (tons)", comp3)]
prow(i) = i <= 2 ? i + 1 : i + 2      # layout rows: 1=headers, 2-3=8.3.5, 4=divider, 5-6=8.5.3
fig = MK.Figure(size = (1550, 1300))
for (ci, (cname, _)) in enumerate(COLS); MK.Label(fig[1, ci+1], cname; fontsize = 19, font = :bold, tellwidth = false); end
for (i, (l3, lu)) in enumerate(STRATA)
  ...
  for (ci, (_, comp)) in enumerate(COLS)
    ax = MK.Axis(fig[r, ci+1]; xticks = (1:nb, binlabels), xticklabelrotation = π/4, ylabel = ci == 1 ? "aggregate" : "")
    xs = Int[]; ys = Float64[]; stk = Int[]; cs = MK.RGBAf[]
    for (si, s) in enumerate(stack_order), b in 1:nb
      y = get(comp, (l3, lu, s, b), 0.0); y > 0 || continue
